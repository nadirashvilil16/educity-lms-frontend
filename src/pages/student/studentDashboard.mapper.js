...
function normalizeLecture(lecture) {
  ...
  return {
    ...
    title: lecture.title || lecture.courseName || lecture.course?.title || "ლექცია",
...
export function mapStudentDashboard(payload = {}, sessionUser = null) {
  const lectures = asArray(payload.lectures).map(normalizeLecture).filter(Boolean);
  const nextLecture = normalizeLecture(payload.nextLecture) || lectures[0] || null;
  ...
  return {
    student: {
      firstName: payload.firstName || payload.student?.firstName || sessionUser?.firstName || "სტუდენტი",
      lastName: payload.lastName || payload.student?.lastName || sessionUser?.lastName || "",
    },
    attendance: {
    ...
    recentGrade: assignments.find((item) => item.submission?.score != null) || null,
...
export const studentDashboardPreview = {
  ...
  nextLecture: { _id: "lecture-1", title: "UI/UX დიზაინი", topic: "აპლიკაციის ინტერაქციული დიზაინი", date: "2026-07-17T17:00:00" },
  lectures: [
    { _id: "lecture-1", title: "UI/UX დიზაინი", topic: "აპლიკაციის ინტერაქციული დიზაინი", date: "2026-07-17T17:00:00" },
    { _id: "lecture-2", title: "UI/UX დიზაინი", date: "2026-07-21T17:00:00" },
    { _id: "lecture-3", title: "UI/UX დიზაინი", date: "2026-07-23T17:00:00" },
  ],
  assignments: [
    { _id: "assignment-5", title: "dashboard UX wireframe", description: "შექმენით სტუდენტის დემო-გვერდის სტრუქტურა", dueDate: "2026-07-21", allowedFileTypes: ["PDF", "PNG", "DRIVE"] },
    { _id: "assignment-4", title: "Color Palette & Typography", submission: { status: "graded", score: 8 } },
  ],
  tasks: [
